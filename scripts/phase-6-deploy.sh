...
say ""
say "That stack needs Alchemy's GitHub provider connected on this machine —"
say "separate from the Cloudflare one from Stage 1 (a real OAuth flow). GitHub"
say "isn't, if 'gh' is already authenticated: Alchemy's own recommended method"
say "just shells out to 'gh auth token', so this is a one-line registration,"
say "not a second login. If demo-project hasn't run this before:"
...
warn "categorically refuses OAuth sessions (what Stage 1's default profile is)."
say "One-time setup, if demo-project hasn't run this before:"
...
pause "In demo-project: run 'ALCHEMY_PROFILE=admin CLOUDFLARE_ACCOUNT_ID=$CLOUDFLARE_ACCOUNT_ID GITHUB_REPO=$DEMO_REPO pnpm bootstrap:github', then Enter when it finishes."
