...
export type Bindings = {
  ...
  // The token is a plain string locally (root `.env` / `alchemy dev` fallback)
  // and a Secrets Store binding (read with `.get()`, see `helpers/secrets.ts`)
  // when deployed (P6-04); the account id is a plain string either way.
