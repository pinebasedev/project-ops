...
- `apps/control-plane`: the Hono API over D1 (Drizzle). It is a **library**, not a deploy target: the dashboard mounts it at `/v1/*` in `apps/dashboard/src/routes/v1/[...rest]/+server.ts`. It has no wrangler config of its own.
- `apps/dashboard`: SvelteKit, client-rendered (`ssr = false`), and the one Worker that is deployed.
...
- Control-plane tests run in Node against an in-memory libsql database. Dashboard tests of `lib/api/controlPlane.ts` run the real control-plane app in-process (`control-plane/testing`), so extend that setup rather than mocking the transport.
- For a schema change, edit `apps/control-plane/src/db/schema.ts` and run `pnpm --filter control-plane db:generate`. Alchemy applies migrations on deploy.
...
- The security invariants every change must keep are in [`REVIEW.md`](./REVIEW.md).
