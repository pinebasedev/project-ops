...
- [x] **P6-01** `feat(alchemy)` `alchemy.run.ts` for the platform's *own* infra: control-plane + dashboard Workers, D1 — the platform starts provisioning itself. Includes applying the control-plane's D1 migrations on deploy and the dashboard's typed-client base URL wiring. *(One stack, `prod` stage. Alchemy owns build/dev/deploy for both apps — `wrangler.jsonc` + `@cloudflare/vite-plugin` removed; `pnpm dev` = `alchemy dev` against local simulators (needs `alchemy profile edit --add Cloudflare` once). The Access resources are `ALCHEMY_DEV`-guarded so `alchemy dev` needs no Zero Trust org (ADR-0009). `pnpm check:alchemy` typechecks; real `alchemy deploy` + P6-06 pending — the founder's job, via `scripts/phase-6-deploy.sh`.)*
