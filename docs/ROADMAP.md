...
- [x] **P6-01** `feat(alchemy)` `alchemy.run.ts` for the platform's *own* infra: control-plane + dashboard Workers, D1 — the platform starts provisioning itself. Includes applying the control-plane's D1 migrations on deploy and the dashboard's typed-client base URL wiring. *(One stack, `prod` stage, deploy-only — local dev stays on `@cloudflare/vite-plugin` / `vite dev` since the Access resources can't be planned without a connected account (ADR-0009). `pnpm check:alchemy` typechecks; `alchemy dev` parses the plan to the first Access resource. Real `alchemy deploy` + P6-06 pending — the founder's job, driven by `scripts/phase-6-deploy.sh`.)*
