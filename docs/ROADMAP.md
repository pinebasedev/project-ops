...
- **The control-plane API and dashboard are never deployed until Phase 6.** The platform provisions its own infra there (P6-01) and it goes up already behind Cloudflare Access (P6-02) — there is no window where a public, unauthenticated control-plane exists on the internet. Phases 1–5 developed and tested them locally: the control-plane via `@cloudflare/vite-plugin` and its unit suite, the dashboard via `vite dev` against a locally-run control-plane. (P6-01 replaced that with `alchemy dev` for both — see ADR-0009.)
...
- Managed-project tickets (marked below) happen in a separate `demo-project` repository — a copy of the Svelteflare boilerplate used as the platform's first managed Project — not this repo.
...
half — the Alchemy `staging` stage and the CI that drives it (P3-01, P3-02) —
was proven for real by the founder, 2026-09-23 (merge into `staging` → deploy,
behind Access). The live Integration Test suite (P3-03) is intentionally
deferred to a future update — not scoped yet; see ADR-0006's update.

- [x] **P3-01** `feat(db)` `Environment.kind` gains `staging` in practice; extend demo-project's Alchemy stack with stage `staging`. *(Control-plane side needs nothing — `kind` already carries `staging` and the query/dashboard paths now exercise it. `staging` is just another value passed to the existing, stage-parameterized `alchemy.run.ts` — no dedicated code needed there beyond P3-02's workflow and the Access gating below. Real `alchemy deploy --stage staging` proven by the founder, 2026-09-23.)*
- [x] **P3-02** `feat(ci)` *(demo-project repo)* Merging a PR into `staging` triggers an Alchemy deploy of stage `staging`; the merged PR's `pr-{number}` stage is destroyed separately by `preview.yml`'s existing `cleanup` job, which already fires on every PR close regardless of target branch. *(`.github/workflows/staging.yml` — triggers on a merged PR into `staging` (`pull_request: closed`, `merged == true`), not a raw `push`, so a direct `git push origin staging` never deploys; deploy only, no destroy step. Also: `staging` (and every `pr-*` stage) now sit behind Cloudflare Access via a per-Worker dedicated application (`alchemy/Access.ts`, `alchemy/Api.ts`'s new `access` param) — interactive Google-IdP login only, founder-allowlisted by default via `CF_ACCESS_ALLOW_EMAIL`/`CF_GOOGLE_IDP_ID`, same pattern as this platform's own P6-02. `prod` is deliberately never gated. Real deploy + Access verified by the founder, 2026-09-23.)*
- [ ] **P3-03** `test(integration)` *(demo-project repo)* The actual Integration Test suite — runs once, live, against the deployed staging URL (ADR-0006). *(Deferred to a future update — see ADR-0006's update. A true local/in-process integration layer, not just this live suite, is wanted alongside it; both land together later rather than partially now.)*
...
- [ ] **P4-01** `feat(db)` `Environment.kind` gains `production`; extend the Alchemy stack with stage `prod`, including the destroy safety guard. *(Control-plane side needs nothing — `kind` already carries `production` and the query/dashboard paths now exercise it. `prod` is just another stage value; the destroy safety guard is structural — neither `production.yml` (P4-02) nor `staging.yml` (P3-02) has a destroy step at all, so there's nothing in CI that could ever target `prod`/`staging` for teardown (`preview.yml`'s own `cleanup` job additionally refuses to destroy those names if `STAGE` ever resolved to one). Remaining: a real `alchemy deploy --stage prod`, verified.)*
- [ ] **P4-02** `feat(ci)` *(demo-project repo)* Merging `staging` → `main` triggers an Alchemy deploy of stage `prod`; callback reports the production Deployment. *(`.github/workflows/production.yml` — triggers on a merged PR into `main` (`pull_request: closed`, `merged == true`), not a raw `push`, for the same reason as staging; deploy only, no destroy step, `kind: "production"`. Goes through the same create/`complete` callback routes as staging; no new control-plane route needed. Unlike `staging`/`pr-*`, `prod` is never put behind Access — it's the live product. `pnpm check:alchemy` typechecks; the real deploy is still pending.)*
...
## Phase 5 — Observability (removed 2026-09, see ADR-0009's update)

P5-01–03 built a recent-errors panel (control-plane's own Cloudflare API
token → Workers Observability Telemetry API → dashboard panel), later removed:
the only runtime use of the platform's Cloudflare API token, and not worth
holding that account-wide credential in a Worker binding when the Cloudflare
dashboard already shows the same logs directly. Kept here only as build
history — the routes, client, and panel no longer exist.

- [x] ~~**P5-01** `feat(observability)` Control-plane's own Cloudflare API token for querying the Workers Observability Telemetry API — `.dev.vars` locally for now (Secrets Store lands in Phase 6).~~
- [x] ~~**P5-02** `feat(observability)` Query route: given an Environment, call Cloudflare's Telemetry API filtered by Worker + time range.~~
- [x] ~~**P5-03** `feat(dashboard)` Recent-errors view: errors for an Environment since its latest Deployment.~~
...
- [x] **P6-01** `feat(alchemy)` `alchemy.run.ts` for the platform's *own* infra: one dashboard + control-plane Worker, D1 — the platform starts provisioning itself. Includes applying the control-plane's D1 migrations on deploy. *(One stack, `prod` stage. Alchemy owns build/dev/deploy for the app — `wrangler.jsonc` + `@cloudflare/vite-plugin` removed; `pnpm dev` = `alchemy dev` against local simulators (needs `alchemy profile edit --add Cloudflare` once). The Access resources are `ALCHEMY_DEV`-guarded so `alchemy dev` needs no Zero Trust org (ADR-0009). `pnpm check:alchemy` typechecks; real `alchemy deploy` + P6-06 pending — the founder's job, via `scripts/phase-6-deploy.sh`. Originally two Workers — control-plane and dashboard — later merged into one, the control-plane API mounted same-origin at `/v1/*` in the dashboard's SvelteKit Worker (ADR-0009); `apps/control-plane` is now a workspace-internal library, not its own deploy target.)*
- [x] **P6-02** `feat(infra)` Cloudflare Access resources in that same stack: `Access.Application` + `Access.Policy` (interactive IdP login) for the dashboard; `Access.ServiceToken` for GitHub Actions callers of the control-plane API (ADR-0005). *(In `alchemy/Access.ts` + `alchemy.run.ts`: `allow-team` (`CF_ACCESS_ALLOW_EMAIL`, default the founder; Google IdP) gates the dashboard via its `access` prop; an explicit `control-plane-access` Application + `allow-ci` non-identity policy gate the API, and its AUD tag is wired into the Worker for P6-03. Google IdP supersedes ADR-0005's GitHub placeholder. Not deploy-verified.)*
- [x] **P6-03** `feat(api)` Access JWT verification middleware in the control-plane Worker — defense in depth, independent of the edge gate (ADR-0005). *(`src/middleware/accessJwt.ts` verifies `Cf-Access-Jwt-Assertion` against the team JWKS on every `/v1` route except `/v1/health`; JWKS cached 1h per isolate. Ungated when `CF_ACCESS_*` unset (local dev). Full unit coverage with an in-runtime RSA keypair.)*
- [x] ~~**P6-04** `feat(infra)` Migrate the control-plane's own fixed secrets to Cloudflare Secrets Store for deployed environments.~~ *(Reversed 2026-09 alongside Phase 5's removal — see ADR-0009's update: with Observability gone, the control-plane Worker has no secret left to store. `alchemy/Secrets.ts` is deleted.)*
- [x] **P6-05** `feat(ci)` *(demo-project repo)* Add `CF_ACCESS_CLIENT_ID`/`CF_ACCESS_CLIENT_SECRET` secrets, sent alongside the existing per-project bearer token on every callback. *(`preview.yml`'s three callback steps now send `CF-Access-Client-Id` / `CF-Access-Client-Secret`. Empty/harmless until the repo secrets are set — done via the deploy wizard. The staging/prod callbacks (P3-02/P4-02) inherit the pattern when they land.)*
- [ ] **P6-06** `feat(infra)` End-to-end verification. With the control-plane + dashboard deployed and Access-gated (P6-01–04) and demo-project sending Access credentials (P6-05): open a real PR on demo-project and confirm the whole chain through Access — provision → control-plane records the Deployment → preview comment → merge into `staging` → integration tests → promote to `main` → prod deploy → dashboard reflects every step. Was the old "P1-09"; deferred to here so the platform's first deployment is never unauthenticated. *(The founder's to run: `scripts/phase-6-deploy.sh` walks connect-account → Zero Trust org + Google IdP → `alchemy deploy` → paste back AUD tag + service-token creds → set demo-project repo secrets → open the PR. Also unblocks the deferred real runs of P3-01/02/03 and P4-01/02.)*
