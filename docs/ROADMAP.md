...
- [x] **P6-03** `feat(api)` Access JWT verification middleware in the control-plane Worker — defense in depth, independent of the edge gate (ADR-0005). *(`src/middleware/accessJwt.ts` verifies `Cf-Access-Jwt-Assertion` against the team JWKS on every `/v1` route except `/v1/health`; JWKS cached 1h per isolate. Ungated when `CF_ACCESS_*` unset (local dev), like `observabilityFromEnv`. Full unit coverage with an in-runtime RSA keypair.)*
- [x] **P6-04** `feat(infra)` Migrate the control-plane's own fixed secrets to Cloudflare Secrets Store for deployed environments. *(Only `CLOUDFLARE_API_TOKEN` is Worker-read and genuinely secret — it moves to the account Secrets Store (`alchemy/Secrets.ts`); `resolveSecret` handles both the local `.dev.vars` string and the deployed `SecretsStoreSecret`. `CLOUDFLARE_ACCOUNT_ID` / `CF_ACCESS_*` stay plain bindings. The runtime binding shape is first exercised on deploy.)*
