...
- **The credentials are a control-plane operational secret**, not a per-Project token: one Cloudflare API token + account ID. Since Phase 6 the token lives in the account Secrets Store on deploy (P6-04) and a plain string locally (root `.env`, optional); the account id is a plain binding. The errors route degrades to a 503 "not configured" when they're absent rather than failing hard.
