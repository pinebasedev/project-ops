...
2. **Perimeter** (can this request reach either Worker at all): Cloudflare Access in front of both the dashboard and the control-plane API, declared as Alchemy resources in the same stack that provisions everything else — not a manual, skippable setup step. The control-plane's Access Application carries both an interactive-login policy (the dashboard's founder-only allowlist) and a Service Token policy (GitHub Actions) — Access alone only answers "some valid credential," not which kind. See [ADR-0005](./docs/adr/0005-cloudflare-access-in-front-of-dashboard-and-api.md).
